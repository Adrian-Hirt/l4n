module NavbarHelper
  def menu_item_active_classes(menu_item)
    if menu_item.lan_party_id.present?
      'active' if "lan/#{menu_item.static_page_name}" == controller_path && params[:id] == menu_item.lan_party_id.to_s
      ...
      'active' if menu_item.children.any? { |child| menu_item_active_classes(child) == 'active' }
